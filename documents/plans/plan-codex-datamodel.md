...
- `module.json` gains:
  ...
  ```
  The resulting type string is `coffee-pub-librarian.codex` (auto-prefixed).
- Localization key: `TYPES.JournalEntryPage.coffee-pub-librarian.codex` → "Codex Entry".
...
`CONFIG.JournalEntryPage.dataModels['coffee-pub-librarian.codex'] = CodexPageModel;`
...
Custom page sheet registered via `DocumentSheetConfig.registerSheet(JournalEntryPage, MODULE.ID, CodexPageSheet, { types: ['coffee-pub-librarian.codex'], makeDefault: true })`.
...
- [x] `documentTypes` in module.json; `TYPES.JournalEntryPage.coffee-pub-librarian.codex` localization
