...
if (!BlacksmithWindowBaseV2) {
    throw new Error('Coffee Pub Librarian | BlacksmithWindowBaseV2 is unavailable for DataExportWindow');
}

export class DataExportWindow extends BlacksmithWindowBaseV2 {
    static ROOT_CLASS = 'librarian-data-export-window';

    static DEFAULT_OPTIONS = foundry.utils.mergeObject(
        foundry.utils.mergeObject({}, super.DEFAULT_OPTIONS ?? {}),
        {
            id: `${MODULE.ID}-data-export`,
            classes: ['librarian-data-export-window', 'librarian-window'],
    ...
    constructor(options = {}) {
        ...
        this.filename = options.filename || 'librarian-export.json';
    ...
    async _onRender(context, options) {
        ...
        const root = this.element?.querySelector?.('.librarian-data-export-window[data-app-id]');
    ...
    _download() {
        try {
            ...
            console.error('Coffee Pub Librarian | Export download failed:', error);
